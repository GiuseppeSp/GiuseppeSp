...
| **Awesome LangWatch Agents** | Curated catalog of 5 runnable AI agents — each ships with LangWatch traces, a hand-labeled golden dataset, pass/fail evaluators, and a measured tuning experiment. Patterns: simple RAG, multi-turn chatbot memory, tool-use, agentic routing, multi-agent pipelines. Half the findings are null results — and the writeups argue that's exactly when the eval framework pays off, because the *next* intervention to test only becomes visible once the obvious one has been ruled out. | Python, OpenAI, LangWatch |
