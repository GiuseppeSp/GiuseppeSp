...
|---------|-------------|------|
| **Customer Interview Synthesizer (n8n)** | Multi-agent n8n workflow that turns raw customer-interview transcripts into prioritized product insights. Four worker agents (themes / JTBDs / contradictions / quotes) run in parallel → synthesizer → LLM-as-judge scores against a hand-curated reference → either auto-save or pause for Slack human-in-the-loop approval. The trust scaffolding (judge + HITL gate + planned drift monitor) is the artifact's main differentiator. | n8n Cloud, OpenAI GPT-4o-mini, Google Sheets, Slack |
| **Nonna's Cookbook RAG** | Agentic recipe assistant inside my iOS app — three-path routing (personal knowledge base, LLM general knowledge, live web search) gated by an LLM-as-judge relevance check. Built-in Observer mode visualizes each pipeline step in real time. Fully instrumented with LangWatch, scored against a hand-labeled golden dataset by three evaluators (routing correctness, gate correctness, LLM-as-judge answer quality) | Flutter, Firebase Cloud Functions, Firestore vector search, OpenAI (GPT-4o + text-embedding-3-small), Tavily, LangWatch |
| **Awesome LangWatch Agents** | Curated catalog of 29 runnable AI agents — each ships with LangWatch traces, a hand-labeled golden dataset, pass/fail evaluators, and a measured A/B tuning experiment. Spans five tiers: foundations (RAG, conversation memory, tool use, agentic routing), reasoning and planning (ReAct, reflexion, chain-of-thought, tree-of-thoughts, ReWOO), advanced RAG (corrective, query rewriting, HyDE, hybrid retrieval + rerank, multi-hop, graph), tools (text-to-SQL, code interpreter, error recovery, tool retrieval, human-in-the-loop), and multi-agent (supervisor-worker, debate, mixture-of-agents). Closes with a synthesis essay on what the 29 experiments add up to. | Python, OpenAI, LangWatch |
| **Model Benchmark** | Blind A/B/C test of frontier LLMs on real PM tasks (synthesize feedback, write briefs, prioritize, analyze competitors, notes-to-decision) — reveals which model wins which workflow, not which wins generic benchmarks | Next.js 16, TypeScript, Tailwind, Anthropic + OpenAI + Google + OpenRouter SDKs, SQLite |
